...
object ModuleListCommand: Command("modulelist", "print a module list", "modulelist") {
	override fun call(args: Array<out String>) {
		println(buildString {
			append("-------------Modules:-------------\n")
			for (module in FeatureManager.getEnabledModules()) {
				append(module.name)
				append(": ")
				...
				//      - Break: Boolean
				//          -- Break Delay: Int
				for (value in Zenith.SettingManager.getPropertiesByMod(module)) {
					append("\t- ")
					append(value.name)
					append(": ")
			...
			append("\n-------------Commands:-------------\n")
			for (command in Zenith.commandManager.commandList) {
